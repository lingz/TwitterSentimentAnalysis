...
public class TwitterDataset {
    ...
    public TwitterDataset(Path filePath) throws IOException, org.json.simple.parser.ParseException {
        ...
        for (int days_after_event = -30; days_after_event < 31; days_after_event++) {
            ...
            for (int i=0; i < tweetCount; i++) {
                JSONObject rawTweet = (JSONObject) rawTweets.get(0);
    ...
    private void process_tweet_sentiment() {
        for (int days_after_event = -30; days_after_event < 31; days_after_event++) {
            ...
            for (Tweet tweet : tweets) {
                ...
                sentimentCount[sentimentArrayIndex] += 1;
            ...
            tweetSentimentMap.put(days_after_event, sentimentCount);
            tweetSentimentWeightedMap.put(days_after_event, sentimentWeights);
        }
    }

    public void writeoutData() throws IOException {
        String newFileName = "processed_" + originalFilePath.getFileName();
    ...
    public void writeoutData(Path filepath) throws IOException {
        ...
        writer.write("Days After Event,Number of Tweets,Negative Tweets, Neutral Tweets,Negative,Positive Proportion" +
                "Negative Weight,Neutral Weight,Positive Weight,Positive Weight Proportion\n");
        for (int days_after_event = -30; days_after_event < 31; days_after_event++) {
            ...
            double positiveProportion = (1.0 * (sentimentCounts[0] + sentimentCounts[1] + sentimentCounts[2])) /
                    numberOfTweets;
            ...
            Integer[] sentimentWeights = tweetSentimentMap.get(days_after_event);
            for (int i=0; i < 3; i++)
                writer.write(sentimentWeights[i] + ",");

            double positiveWeightProportion = (1.0 * (sentimentWeights[0] + sentimentWeights[1] + sentimentWeights[2])) /
                    totalWeight;
